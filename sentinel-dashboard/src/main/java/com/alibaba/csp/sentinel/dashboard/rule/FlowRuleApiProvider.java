...
public class FlowRuleApiProvider implements DynamicRuleProvider<List<FlowRuleEntity>> {
	...
	public List<FlowRuleEntity> getRules(String appName) throws Exception {
		...
		List<MachineInfo> list = appManagement.getDetailApp(appName).getMachines()
				.stream().filter(MachineInfo::isHealthy).sorted((e1, e2) -> Long
						.compare(e2.getLastHeartbeat(), e1.getLastHeartbeat()))
		...
		if (list.isEmpty()) {
			...
			return sentinelApiClient.fetchFlowRuleOfMachine(machine.getApp(),
