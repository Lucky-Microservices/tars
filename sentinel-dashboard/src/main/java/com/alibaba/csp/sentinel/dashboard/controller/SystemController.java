...
public class SystemController {
	...
	public Result<List<SystemRuleEntity>> apiQueryMachineRules(HttpServletRequest request,
			String app) {
	...
	public Result<SystemRuleEntity> apiAdd(HttpServletRequest request, String app,
			Double highestSystemLoad, Double highestCpuUsage, Long avgRt, Long maxThread,
			Double qps) {
		AuthUser authUser = authService.getAuthUser(request);
		...
		if (notNullCount != 1) {
			return Result.ofFail(-1,
					"only one of [highestSystemLoad, avgRt, maxThread, qps,highestCpuUsage] "
							+ "value must be set > 0, but " + notNullCount
							+ " values get");
		}
