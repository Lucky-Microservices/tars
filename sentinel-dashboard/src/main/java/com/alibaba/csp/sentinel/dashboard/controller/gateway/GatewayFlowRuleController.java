...
public class GatewayFlowRuleController {
	...
	private final Logger logger = LoggerFactory
			.getLogger(GatewayFlowRuleController.class);
	...
	public Result<List<GatewayFlowRuleEntity>> queryFlowRules(HttpServletRequest request,
			String app) {
	...
	public Result<GatewayFlowRuleEntity> addFlowRule(HttpServletRequest request,
			@RequestBody AddFlowRuleReqVo reqVo) {
		AuthService.AuthUser authUser = authService.getAuthUser(request);
		...
		if (!Arrays.asList(RESOURCE_MODE_ROUTE_ID, RESOURCE_MODE_CUSTOM_API_NAME)
		...
		if (paramItem != null) {
			...
			if (!Arrays.asList(PARAM_PARSE_STRATEGY_CLIENT_IP, PARAM_PARSE_STRATEGY_HOST,
			...
			if (Arrays.asList(PARAM_PARSE_STRATEGY_HEADER, PARAM_PARSE_STRATEGY_URL_PARAM,
				...
				if (StringUtil.isNotEmpty(pattern)) {
					...
					if (!Arrays.asList(PARAM_MATCH_STRATEGY_EXACT,
							PARAM_MATCH_STRATEGY_CONTAINS, PARAM_MATCH_STRATEGY_REGEX)
							.contains(matchStrategy)) {
						return Result.ofFail(-1,
								"invalid matchStrategy: " + matchStrategy);
					}
		...
		if (!Arrays.asList(INTERVAL_UNIT_SECOND, INTERVAL_UNIT_MINUTE, INTERVAL_UNIT_HOUR,
				INTERVAL_UNIT_DAY).contains(intervalUnit)) {
		...
		if (!Arrays.asList(CONTROL_BEHAVIOR_DEFAULT, CONTROL_BEHAVIOR_RATE_LIMITER)
				.contains(controlBehavior)) {
		...
		if (CONTROL_BEHAVIOR_DEFAULT == controlBehavior) {
			...
			if (maxQueueingTimeoutMs < 0) {
				return Result.ofFail(-1,
						"invalid maxQueueingTimeoutMs: " + maxQueueingTimeoutMs);
			}
