...
public class ParamFlowRuleController {
	...
	private boolean checkIfSupported(String app) {
		try {
			return Optional.ofNullable(appManagement.getDetailApp(app))
					.flatMap(e -> e.getMachines().stream().filter(MachineInfo::isHealthy)
							.findFirst())
	...
	public Result<List<ParamFlowRuleEntity>> apiQueryAllRulesForMachine(
			HttpServletRequest request, @RequestParam String app) {
	...
	private <R> Result<R> checkEntityInternal(ParamFlowRuleEntity entity) {
		...
		if (entity.getGrade() != RuleConstant.FLOW_GRADE_QPS) {
			return Result.ofFail(-1,
					"Unknown mode (blockGrade) for parameter flow control");
		}
	...
	public Result<ParamFlowRuleEntity> apiUpdateParamFlowRule(HttpServletRequest request,
		...
		try {
			...
			logger.error("Error when updating parameter flow rules, id=" + id,
					ex.getCause());
			if (isNotSupported(ex.getCause())) {
				return unsupportedVersion();
			}
