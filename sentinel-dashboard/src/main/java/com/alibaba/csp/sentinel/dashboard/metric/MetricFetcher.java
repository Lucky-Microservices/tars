...
public class MetricFetcher {
	...
	private static final Charset DEFAULT_CHARSET = Charset
			.forName(SentinelConfig.charset());
	...
	@Autowired
	@Qualifier("openTSDBMetricsRepository")
	...
	private ScheduledExecutorService fetchScheduleService = Executors
	...
	public MetricFetcher() {
		...
		fetchService = new ThreadPoolExecutor(cores, cores, keepAliveTime,
				TimeUnit.MILLISECONDS, new ArrayBlockingQueue<>(queueSize),
				new NamedThreadFactory("sentinel-dashboard-metrics-fetchService"),
				handler);
		fetchWorker = new ThreadPoolExecutor(cores, cores, keepAliveTime,
				TimeUnit.MILLISECONDS, new ArrayBlockingQueue<>(queueSize),
				new NamedThreadFactory("sentinel-dashboard-metrics-fetchWorker"),
				handler);
		IOReactorConfig ioConfig = IOReactorConfig.custom().setConnectTimeout(3000)
				.setSoTimeout(3000)
		...
		httpclient = HttpAsyncClients.custom()
				...
				}).setMaxConnTotal(4000).setMaxConnPerRoute(1000)
				.setDefaultIOReactorConfig(ioConfig).build();
		httpclient.start();
		start();
	}
	...
	private void fetchOnce(String app, long startTime, long endTime, int maxWaitSeconds) {
		if (maxWaitSeconds <= 0) {
			throw new IllegalArgumentException(
					"maxWaitSeconds must > 0, but " + maxWaitSeconds);
		}
		...
		for (final MachineInfo machine : machines) {
			...
			if (machine.isDead()) {
				latch.countDown();
				appManagement.getDetailApp(app).removeMachine(machine.getIp(),
						machine.getPort());
				logger.info("Dead machine removed: {}:{} of {}", machine.getIp(),
						machine.getPort(), app);
				continue;
			}
			...
			final String url = "http://" + machine.getIp() + ":" + machine.getPort() + "/"
					+ METRIC_URL_PATH + "?startTime=" + startTime + "&endTime=" + endTime
					+ "&refetch=" + false;
			...
			httpclient.execute(httpGet, new FutureCallback<HttpResponse>() {
				...
				public void failed(final Exception ex) {
					...
					if (ex instanceof SocketTimeoutException) {
						logger.error("Failed to fetch metric from <{}>: socket timeout",
								url);
					}
	...
	private void doFetchAppMetric(final String app) {
		...
		try {
			...
			logger.info("submit fetchOnce(" + app + ") fail, intervalMs [" + lastFetchMs
					+ ", " + endTime + "]", e);
		}
	}
	...
	private void handleBody(String[] lines, MachineInfo machine,
			Map<String, MetricEntity> map) {
		...
		for (String line : lines) {
			try {
				 ...
				 */
				String key = buildMetricKey(machine.getApp(), node.getResource(),
						node.getTimestamp());
				...
				logger.warn("handleBody line exception, machine: {}, line: {}",
						machine.toLogString(), line);
			}
		}
	}
