...
public class GatewayApiController {
	...
	public Result<List<ApiDefinitionEntity>> queryApis(HttpServletRequest request,
			String app) {
	...
	public Result<ApiDefinitionEntity> addApi(HttpServletRequest request,
			@RequestBody AddApiReqVo reqVo) {
		AuthService.AuthUser authUser = authService.getAuthUser(request);
		...
		for (ApiPredicateItemVo predicateItem : predicateItems) {
			...
			if (!Arrays.asList(URL_MATCH_STRATEGY_EXACT, URL_MATCH_STRATEGY_PREFIX,
					URL_MATCH_STRATEGY_REGEX).contains(matchStrategy)) {
		...
		// 检查API名称不能重复
		List<ApiDefinitionEntity> allApis = repository.findAllByApp(app.trim());
		if (allApis.stream().map(o -> o.getApiName())
				.anyMatch(o -> o.equals(apiName.trim()))) {
	...
	public Result<ApiDefinitionEntity> updateApi(HttpServletRequest request,
			@RequestBody UpdateApiReqVo reqVo) {
		AuthService.AuthUser authUser = authService.getAuthUser(request);
